...
namespace CourseLibrary.API.Controllers
{
    ...
    public class CoursesController : ControllerBase
    {
        ...
        public ActionResult PartiallyUpdateCourseForAuthor(Guid authorId, Guid courseId, JsonPatchDocument<CourseForUpdateDto> patchDocument)
        {
            ...
            if (courseForAuthorFromRepo == null)
            {
                var coutseDto = new CourseForUpdateDto();
                patchDocument.ApplyTo(coutseDto, ModelState);

                if (!TryValidateModel(coutseDto))
                {
                    return ValidationProblem(ModelState);
                }

                var courseToAdd = _mapper.Map<Entities.Course>(coutseDto);
                courseToAdd.Id = courseId;
